casa = input("Digite uma casa:")

if casa != "Baratheon" and casa != "Targaryen" and casa != "Tyrell" and casa != "Stark" and casa != "Lannister" and casa != "Greyjoy" and casa != "Tully" and casa != "Arryn" and casa != "Martell":
	print ("Entrada", casa , "invalida")
elif casa == "Baratheon" :
	print ("Nossa e a furia")
elif casa == "Targaryen":
	print ("Fogo e sangue")
elif casa == "Tyrell": 
	print ("Crescendo fortes")
elif casa == "Stark": 
	print ("O inverno esta chegando")
elif casa == "Lannister": 
	print ("Oucam-me rugir")
elif casa == "Greyjoy": 
	print ("Nos nao semeamos")
elif casa == "Tully":
	print ("Familia, dever, honra")
elif casa == "Arryn" :
	print ("Tao alto como a honra")
else:
	print ("Insubmissos, nao curvados, nao quebrados")
	
   
	