...
class VisitorMC3(ast.NodeVisitor):
    def __init__(self):
        ...
        self.unusedImports = []                 #import nao utilizado (A5)
        self.repeatedCommandsInIfs = False      #comandos repetidos em ifs (B4)
        self.redundantOpsInLoop = False         #operacoes redundantes em loops (C3)

        self.selfAssignment = False             #variavel atribuida a si mesma (A2)
        self.unnecessaryElifElse = False        #elif/else desencessario (B10)
        self.sameBodyIfs = False                #ifs distintos com blocos de codigo iguais (B11)
        self.unusedInitVar = False              #variavel inicializada desnecessariamente (A3)
        self.excessiveCombinationChecks = False #verificacao desnecessaria de todas as possibilidades (E1)
    ...
    def checkIfsWithSameBody(self, root):
        ...
        Verifica se blocos de elif distintos possuem o mesmo conteudo'''
        for node in ast.walk(root):
            if isinstance(node, ast.If) and isinstance(node.parent, ast.Module):
                prev_body_repr = None
                for child in ast.iter_child_nodes(node.parent):
                    if isinstance(child, ast.If):
                        current_body = "|".join([ast.dump(stmt) for stmt in child.body])
                        if prev_body_repr == current_body:
                            self.sameBodyIfs = True
                            return
                        prev_body_repr = current_body

    def checkUnnecessaryElifElse(self, root):
        """"B10 - elif/else desnecessário.
        Verifica se existe um elif e/ou um else desnecessário"""
        for node in ast.walk(root):
            if isinstance(node, ast.If):
                if not node.body and node.orelse:
                    ...
                    if isinstance(node.orelse[0], ast.If):
                        if not node.orelse[0].body:
                            self.unnecessaryElifElse = True

    def checkSelfAssignment(self, root):
        """"A2 - Variavel atribuida a si mesma.
        Verifica se a variavel recebeu o mesmo valor que já tinha anteriormente"""
    ...
    def checkRedundantOpsInLoop(self, root):
            ...
            Verifica se há operações idênticas repetidas dentro de um mesmo laço.
            """
            for node in ast.walk(root):
    ...
    def checkUnusedImports(self, root):
        ...
        Verifica se há módulos importados que não são utilizados no código.
        """
        import_names = set()
        used_names = set()
